...
| FIT-027 | Cached target-side SSIM statistics | todo — measured opportunity, not started; SSIM is the largest term in the tiled fit step (512²: SSIM fwd+bwd `6.531 ms` vs L1 `0.206 ms`, Adam `~0.19 ms`); separable window already landed with PORT-002 (`1.36--1.48x` at ≥256²), caching the fixed target's `mu_t`/`sig_t` measures a further `1.54--1.87x` with `2.0e-6` relative gradient agreement (C55); needs per-fit state plus invalidation on pyramid/curriculum target changes | PORT-002, FIT-003, FIT-016 |
| CORE-012 | One maintained entrypoint for the current best pipeline | implemented (ADR-0025) — `structsplat.pipeline.run_pipeline` + `structsplat convert`; masked and full-frame arms over one safe-commit schedule, arm selected by `mask`; recipe defaults are the measured best (C25/C50/C51/C52) and do not change any library default; full-frame arm unscreened pending BENCH-017 | FIT-023/024/025, CORE-010/011, INIT-009, ADR-0013/0017/0019/0025 |
