...
def archive(outdir, bundles, *, timing_sidecar=None):
    ...
    inventory = {'schema': 'structsplat.overnight.partial-archive.v1', 'complete_replay_bundle': False,
                 'packager_source_sha256': digest(__file__),
                 'selection': selection.__doc__, 'bundles': {}}
    sections = []
    if timing_sidecar is not None:
    ...
    for label, root, manifest, rows in inspected:
        ...
        inventory['bundles'][label] = {
            ...
            'scope': 'All decision/scalar/configuration/history/native-field evidence; selected seed0 display examples. '
        ...
        page += '<p><a href="../index.html">Archive index</a> · <a href="decision.json">Frozen decisions</a> · '
        ...
        (destination / 'index.html').write_text(document(page + table + '<h2>Illustrative displays, not a selection gate</h2>' + figures))
    ...
    intro += 'and independent audit for interpretation and limitations.</p><p><a href="archive_manifest.json">'
    ...
    (outdir / 'index.html').write_text(document(intro))
...
def document(body):
    return ('<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" '
            'content="width=device-width,initial-scale=1"><title>StructSplat overnight evidence</title>'
...
def main():
    ...
    parser.add_argument('--bundle', action='append', required=True, help='LABEL=PATH')
    parser.add_argument('--timing-sidecar', help='Completed external occupancy sidecar directory')
    args = parser.parse_args()
    archive(args.outdir, [value.split('=', 1) for value in args.bundle], timing_sidecar=args.timing_sidecar)
