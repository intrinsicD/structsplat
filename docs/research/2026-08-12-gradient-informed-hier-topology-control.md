...
| StructSplat `fit.py` | post-reduction absolute mean-gradient EMA, residual, support, responsibility | selector chooses rows; configured wave fixes the edit | existing differentiable 2D signals | the current `absgrad` proxy cannot see pixel cancellation and there is no common action currency |
