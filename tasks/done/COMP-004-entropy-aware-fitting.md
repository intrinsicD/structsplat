...
**Status: done.** Completed 2026-07-07. Fit-time QAT modes, `lambda_rate`, a
differentiable rate proxy, returned frozen codec config, CLI flags, tests, and a matched
lambda-sweep RD benchmark are implemented. The result is kept searchable, not promoted to the
default codec path.
...
- [x] Differentiable bit estimate covers positions, scales, rotations, colors, and opacity if
      enabled.
- [x] Final encoded file uses the same quantizer assumptions optimized during fitting in the
      fit-time QAT RD path via `qat_codec_config`.
- [x] Tests show fit-time QAT improves low-bit reconstruction over direct post-hoc quantization on
      a tiny fixture.
- [x] `benchmarks/rate_distortion.py` can sweep lambda and write RD evidence for each point.
- [x] COMP-003 rungs that remain codec-only are kept compatible with this fit-time mode.
...
- Smoke/plumbing evidence: `ara/evidence/comp004-fit-time-qat-smoke-2026-07-07/run.md`.
- Lambda sweep: `ara/evidence/comp004-lambda-sweep-2026-07-07/run.md`.
- The 4-image, 4-ladder sweep found that fit-time STE QAT improves encoded low/mid-bit PSNR over
  direct post-hoc quantization, but it does not beat the existing post-fit QAT control overall.
  Existing post-fit QAT averaged +0.6580 dB over direct encode, while the best fit-time lambda
  averaged +0.6539 dB. Keep `qat_mode` / `lambda_rate` available for future codec experiments;
  do not promote as a default.
