...
3. **The full-frame arm degenerates the mask machinery rather than replacing it.** `mask=None`
   ...
   image diagonal, so the projection interior is the whole frame and per-row caps are inert.
   Boundary initialization, containment, losses, metrics, and proposals are disabled. The same
   closure slot and step budget use count-matched general coverage/detail proposals, so every
   non-boundary mechanism, the Pareto commit gate, and the metric vector are shared verbatim
   (ADR-0027).
...
ADR-0020..0023 (pooled/transactional storage), ADR-0027 (workflow and full-frame parity),
ADR-0011 (renderer). Does not supersede ADR-0009:
