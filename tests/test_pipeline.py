...
@pytest.mark.slow
class TestEndToEnd:
    def test_full_frame_arm_runs_general_closure(self):
        ...
        assert "general_closure" in phases
        assert not [
            record
    ...
    def test_both_arms_share_the_schedule_and_the_commit_gate(self):
        ...
        masked_schedule = dict(masked["schedule"])
        full_schedule = dict(full["schedule"])
        assert masked_schedule.pop("boundary_enabled") is True
        assert full_schedule.pop("boundary_enabled") is False
        masked_boundary = dict(masked_schedule["boundary"])
        full_boundary = dict(full_schedule["boundary"])
        assert masked_boundary.pop("name") == "boundary_closure"
        assert full_boundary.pop("name") == "general_closure"
        masked_schedule["boundary"] = masked_boundary
        full_schedule["boundary"] = full_boundary
        assert masked_schedule == full_schedule
