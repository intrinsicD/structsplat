...
3. **Full-frame arm by degeneration, not duplication.** `run_safe_schedule(mask=None)` builds an
   ...
   empty, and `_visible_boundary` is empty. Boundary closure is skipped explicitly and recorded as
   a `phase_skipped` history event.
...
- [x] `mask=None` skips boundary closure and records the skip; the masked arm still runs it.
- [x] Both arms produce the same `schedule` and share every non-boundary phase.
