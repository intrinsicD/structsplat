...
**Research prompt used:** [`docs/prompts/real-research.md`](../../docs/prompts/real-research.md)
...
The most defensible near-term opportunities are therefore: (a) ownership-aware diagnostics and
recovery-response modeling, (b) a certified bounded-contributor renderer, (c) exact gauge
invariance as an allocator correctness test rather than a promoted method, and (d) actual-rate
tests of richer local color/atom grammars. The exact SAD
...
Three bounded experiments accompany this report:
...
**Killing result:** Leave-one-image-out prediction no better than ranking by immediate gain. This
program survives because FIT-017 has already supplied a nontrivial reversal to explain.
...
**Detailed report:** [`2026-07-15-opacity-gauge-experiment.md`](../../docs/research/2026-07-15-opacity-gauge-experiment.md)
...
## 12. Recommended first experiment after FIT-019

E1/T1 is now resolved: gauge sensitivity is real, but invariant allocation does not improve
recovery consistently. The cheapest genuinely different next question is **E2 perturb--recover
response spectroscopy**, not another alpha or quotient sweep.

- **Central claim:** Early response descriptors predict whether a fixed-count intervention will
  improve or reverse under late recovery.
- **Independent variable:** Replace repeated raw tickets one at a time with the next distinct
  physical group, giving controlled coverage levels from five through eight.
- **Controls:** Same canonical checkpoint, eight births, field count, optimizer restart, target,
  action primitive, and total steps.
- **Measurements:** Dense recovery at `0,1,2,5,10,20,40,60,100,200`; early slope, peak gain,
  crossover, asymptote and AUC; residual concentration, gradient alignment, empirical curvature
  and parameter displacement.
- **Data:** Expand sinusoid frequency/phase/orientation and ramp direction with enough seeds for
  target-family inference; do not consume protected natural images.
- **Decisive observation:** A preregistered trajectory descriptor predicts held-out late recovery
  beyond target-family and intervention-strength baselines.
- **Abandonment:** Stop if the descriptor fails held-out calibration or coverage remains subordinate
  to target identity.

Per-Gaussian backward (performance) and marginal-stream RD/compact luminance slope
(compression/expressiveness) remain independent tracks. FIT-019 supplies no reason to bundle them
with quotient state.
...
|---|---|---|
| Quality | SAD alpha 0.7 lost `-0.0198 dB` post-20 to support. Gauge-quotient alpha 1 gained `+0.2111 dB` post-20 over raw gauge selection but won only 5/8 families, missed support by `-0.0665 dB`, and lost `-0.6007 dB` post-100. | No promoted improvement; donor and quotient allocators rejected by their guards. |
| Convergence | FIT-019 confirms allocation non-commutation but shows post-20/post-100 sign reversals in 7/16 cells; endpoint means are target-composition-sensitive. | Run response spectroscopy; do not expand the quotient endpoint study. |
| Performance | K=16 exposes a 49.59% positive-contribution oracle ceiling on one field; FIT-019 timing changes sign across CPU replay. | Promising work bound, **no measured speedup**. Profile exact backward independently. |
| Compression | No new stream was built. BENCH-007's actual-rate tensor-WSE gate remains negative; SGI shows entropy modeling, not structure alone, is decisive. | No improvement demonstrated. Test marginal actual bytes and a lightweight context model next. |
| Expressiveness | Existing affine-color smoke is positive (~+1.06 dB) but lacks equal-scalar/equal-byte, CUDA-backward and codec evidence; WIPES/NeuRBF occupy richer local appearance. | Promising but unconfirmed. Test compact luminance slope and WIPES at equal actual bytes/work. |
